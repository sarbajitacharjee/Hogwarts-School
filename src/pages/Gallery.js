...
const sampleData = [
  ...
  { id: 3, type: 'event', date: '2024-01-01', media: schoolevent6 },
  { id: 4, type: 'activity', date: '2024-01-01', media: schoolevent7 },
  { id: 5, type: 'activity', date: '2024-01-01', media: schoolevent4 },
  // Add more items
];
...
const GalleryPage = () => {
  ...
  useEffect(() => {
    // Clear the filteredItems state before applying new filters
    setFilteredItems([]);

    // Filter items based on selected filters
    const { type, date } = filters;
    const result = items.filter(
      item => (!type || item.type === type) && (!date || item.date === date)
    );
